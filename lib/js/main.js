...
let adjacentNodes = ["row0_0", "row0_1", "row1_0", "row1_1"];

rows(12, cellColors, adjacentNodes);

function rows(numberOfRows, cellColors, adjacentNodes) {
  let rowHTML = "";
  ...
  changeRowHeight(numberOfRows, cellColors, adjacentNodes);
}

function changeRowHeight(numberOfRows, cellColors, adjacentNodes) {
  var id = "";

  for (var i = 0; i < numberOfRows; i++) {
    id = "row" + i;
    var row = document.getElementById(id);
    row.style.height = boardWidth / numberOfRows + "px";

    // Adding cells to every row.
    squaresPerRow(id, i, numberOfRows, cellColors, adjacentNodes);
  }
}

function squaresPerRow(
  rowId,
  colId,
  numberOfSquares,
  cellColors,
  ...
  for (let i = 0; i < numberOfSquares; i++) {
    ...
    cell.addEventListener("click", event => {
      ...
      if (adjacentNodes.includes(currentNodeId)) {
